...
class Family(object):
    ...
    def get_antec(self, node):
        """
        Returns a list of ancestors 
        node -- string of node
        """  
        lst = []
        p = self.names_to_nodes[node]
        while (True):
            if p == None:
                break
            else:
    ...
    def cousin(self, a, b):
        ...
        antec_a = self.get_antec(a)
        antec_b = self.get_antec(b)
        #print len(antec_a), len(antec_b)
                         
        if self.names_to_nodes[a] in antec_b:
            return (-1, len(antec_b) - len(antec_a))
        if self.names_to_nodes[b] in antec_a:
            return (-1, len(antec_a) - len(antec_b))
         
        i, j = 0, 0           
        if len(antec_a) >= len(antec_b):
           for antec in antec_b:
               if antec in antec_a:
                    i += 1
           return (len(antec_b) - 2 - (i-2), len(antec_a) - len(antec_b))
        else:
           for antec in antec_a:
               if antec in antec_b:
                    j += 1
           return (len(antec_a) - 2 - (j-2), len(antec_b) - len(antec_a))
